...
{
	// 기본적으로 Tick을 꺼버리자:
	// PawnExtensionComponent는 이벤트 방식으로 초기화 로직을 사용하기 때문에 Tick으로 주기적으로 로직을 할 필요가 없기 때문에 Tick을 끕니다.
...
/*
 *OnRegister()는 액터가 생성되고 Component로 Attach후 초기화가 되었을 때 가장 먼저 호출되는 함수입니다. 그 이후에 불리는게 BeginPlay()함수 입니다.
OnRegister 함수는 RegisterInitStateFeature()를 호출하여 컴포넌트 관리자에게 PawnExtensionComponent을 등록합니다.
GameFeature는 나중에 자세히 알아볼 예정이고 지금은 PawnComponent를 하나의 Feature 단위로 생각하면 됩니다.
간단하게 예를 들어보면 PhysicsComponent, AudioComponent 이런 것들을 생각해보면 기능 단위의 Component로서 Actor에 Physics와 Audio의 Feature를 할당하는 것으로 생각하면 됩니다.
 */
...
void ULyraClonePawnExtensionComponent::BeginPlay()
{
	...
	/*
	 *BindOnActorInitStateChanged()는 Feature Component의 InitState 상태를 관찰하는 함수 입니다.
	NAME_None을 넣으면, 나를 제외한 다른 컴포넌트들의 상태 변화가 있으면 다 알려달라는 뜻 입니다. 
	FGameplayTag()를 넣으면 모든 InitState상태를 전부 알려달라는 뜻 입니다.
	...
	// TryToChangeInitState()는 상태를 변경을 요청하는 함수 입니다.
...
{
	// CheckDefaultInitialization()는 forceUpdate 즉 모든 상태 부분에 대해서 강제적으로 업데이트 해주는 함수 입니다. 
	// CheckDefaultInitializationForImplementers()는 나를 제외하고 전부 CheckDefaultInitialization을 호출해주는 함수 입니다.
	...
	// ContinueInitStateChain()는 State변경이 가능할 때까지 계속 변경하는 함수 입니다.
